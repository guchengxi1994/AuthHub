...
3. 再实现 Admin Web。
4. [x] 提供独立 `auth-hub-client` Python SDK，包含模块清单注册与 FastAPI 路由鉴权依赖。
5. [x] 提供独立 `@auth-hub/react` 轻量渲染 SDK，支持会话权限快照、路由守卫、UI 操作/组件守卫和菜单过滤。
...
- [x] 业务模块是上游业务边界，不等同于前端模块、页面或组件。
- [x] 资源必须归属业务模块；资源类型固定为 API 接口、业务实体、页面/菜单、UI 操作、UI 组件和自定义资源。
- [x] 权限是资源上的操作，必须同时关联模块、资源和动作；动作初版为查看、读取、创建、更新、删除、执行和管理。
- [x] 角色是权限集合，用户通过角色获得权限；用户可绑定多个组织。
- [x] Admin Web 自动生成内部模块 ID、角色编码和权限编码；SDK/上游注册保留显式技术标识以支持幂等同步。
- [x] 上游业务服务通过注册密钥同步资源清单；浏览器只携带用户 Bearer Token 到业务后端。
...
- [x] 支持 API/接口权限元数据。
- [x] 支持操作权限。
- [x] 资源权限字段：权限关联资源 ID、资源类别、资源标识和动作。
- [x] 明确资源级授权输入模型：`module_id + resource_id + action`。
- [ ] 不固定权限编码格式。
...
- [x] `auth-hub-client[sqlalchemy]` 提供事务型 Outbox、提交后即时投递、指数退避、死信保留和手动重试；业务服务将 Outbox 表纳入自己的迁移。
- [~] 跨服务创建/更新可通过 SDK Outbox 保证最终一致性；消息队列、Celery 等特定运行器仍由业务服务自行接入。
...
- [x] 角色管理页。
- [x] 模块管理页。
- [x] 独立权限管理页：按业务模块、资源和操作创建权限，并可直接授予角色。
- [x] 角色权限配置页。
- [x] 用户角色、用户组织配置页。
- [ ] 基础系统配置页。
- [x] 登录态、Token 过期和退出处理。
...
- [ ] 评审模块注册幂等规则。
- [x] 确认 SDK 的目标调用方式：Python SDK 负责后端执行鉴权，React SDK 负责消费权限快照并控制前端渲染。
...
- [x] Tailwind CSS 基础布局。
- [x] admin 登录。
- [x] 用户、组织、角色、权限、模块、资源、审计页面及关系配置入口。
- [x] 权限分配交互。
- [x] 前后端同服务部署。
- [ ] 生产构建和静态资源发布。
...
- [~] 从正式 OpenAPI/契约生成或实现 SDK；当前为稳定 MVP 手写客户端。
- [x] Python 同步客户端。
- [ ] Python 异步客户端。
- [ ] Token 注入与请求上下文。
- [ ] 单个/批量鉴权封装。
...
### Phase 5.1：React 权限渲染 SDK

- [x] 独立 `web-sdk/` npm 包 `@auth-hub/react`。
- [x] 登录后一次加载权限快照，支持显式刷新和会话切换。
- [x] `PermissionRoute` 路由守卫。
- [x] `Permission` / `PermissionButton` UI 操作和组件守卫。
- [x] `filterByPermission` 动态菜单过滤。
- [ ] npm 依赖安装后的 TypeScript、Vitest 和生产打包验证。
...
| Admin Web | MVP 完成 | 基础配置闭环已实现，仍缺分页、细粒度管理权限和端到端覆盖 |
| 审计 | 原型完成 | 只有内存日志，缺查询、持久化和脱敏 |
| 测试 | 初版完成 | 缺 API、基础设施契约、安全和性能测试 |
| SDK | MVP 完成 | Python 鉴权/注册和 React 权限渲染包已实现，仍缺异步客户端、兼容矩阵和发布流程 |
