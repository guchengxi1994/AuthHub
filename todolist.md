...
当前仓库已经完成的是一个“框架核心原型”，不是设计文档要求的完整 AuthHub。已经具备领域模型、认证/RBAC 基础用例、可注入端口、内存适配器和部分 FastAPI API；数据库/Redis 生产适配器、完整管理 API、Admin Web、正式 Token 方案、完整审计、OpenAPI 契约、SDK 等仍未完成。
...
- [ ] 提供生产级 Session/Opaque Token 实现选项。
- [ ] 支持外部密钥/密钥轮换服务。
...
- [ ] 权限不存在时返回 `PERMISSION_NOT_FOUND` 或明确采用隐式拒绝策略。
- [ ] 权限启用/禁用。
- [ ] 权限编码唯一约束。

### 3.6 Resource 与未来扩展字段

- [~] ResourceDefinition 初步模型。
- [ ] 资源实例持久化模型。
...
- [ ] 统一请求/响应 envelope。
- [ ] 统一错误响应结构。
- [ ] 明确 401、403、404、409、422、429、500 映射。
- [ ] 登录失败次数限制。
- [ ] 验证码能力。
- [ ] 暴力破解防护。
- [ ] 登录设备/会话管理。
...
- [x] 返回 user_id。
- [x] 返回 reason。
- [~] 区分未认证、用户不存在、用户禁用、Token 无效、权限不足。
- [ ] 区分权限不存在。
- [x] 区分系统管理员。
...
- [ ] 完整缓存 Key 设计，例如：
  ...
  - [ ] `authhub:user-permissions:{user_id}:v{version}`
  - [ ] `authhub:role-permissions:{role_id}:v{version}`
  - [ ] `authhub:token:{token_id}`
  - [ ] `authhub:blacklist:{token_id}`
- [ ] 缓存版本号或权限版本号。
- [ ] 角色影响用户集合的失效策略。
- [ ] 模块/权限同步后的失效策略。
- [ ] Redis 故障时回源数据库策略。
...
- [~] resource 模块，目前只有模型和注册字段。
- [~] token 模块，目前以 TokenService 端口和内存实现存在。
- [~] audit 模块，目前以 AuditLog 端口和内存实现存在。
...
| Redis | 未完成 | 只有 Cache 协议和内存实现 |
| 认证 | 原型完成 | Token 不是生产实现，缺少安全策略 |
| 授权 | 原型完成 | 权限不存在、资源权限、策略解释未完成 |
| 动态模块 | 原型完成 | 幂等、版本、事务、菜单、同步结果未完成 |
| 管理 API | 初版完成 | 缺分页、删除、查询详情、完整权限控制 |
...
- [ ] Token/Session 策略冻结并有生产实现。
- [ ] 数据库和 Redis 适配器通过契约测试。
- [ ] 用户、组织、角色、权限、模块管理 API 完整可用。
- [ ] 鉴权接口覆盖 Bearer Token、批量检查和权限不存在场景。
- [ ] 用户禁用、角色权限变化能够立即影响授权结果。
- [ ] Admin Web 能完成基础配置闭环。
